...
def cavar(m, m2,coordsMina):
    print("Introduce las coordenadas que quieras cavar:")
    coords = int(input("X: ")), int(input("Y: "))
    m[coords[1]][coords[0]] = "⛏"
    m2[coords[1]][coords[0]] = "⛏"

    dx = coords[0] - coordsMina[0]
    dy = coords[1] - coordsMina[1]

    if (dx == 1 or dx == -1) and dy == 0:
        print("Peligro. Hay una mina cerca")
    elif (dy == 1 or dy == -1) and dx == 0:
        print("Peligro. Hay una mina cerca")
    elif (dx == 1 or dx == -1) and (dy == 1 or dy == -1):
        print("Peligro. Hay una mina cerca")
...
while not fin:
    coordsCavar = cavar(mapaVacio, mapa, coordsMina)
